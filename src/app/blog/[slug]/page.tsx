...
const mdxComponents = {
  h1: (props: React.ComponentProps<"h1">) => (
    <h1 className="text-3xl sm:text-4xl font-bold text-brand-navy mt-10 mb-4" {...props} />
  ),
  h2: (props: React.ComponentProps<"h2">) => (
    <h2 className="text-2xl sm:text-3xl font-bold text-brand-navy mt-10 mb-4" {...props} />
  ),
  h3: (props: React.ComponentProps<"h3">) => (
    <h3 className="text-xl font-semibold text-brand-navy mt-8 mb-3" {...props} />
  ),
  p: (props: React.ComponentProps<"p">) => (
    <p className="text-text-secondary leading-relaxed my-4" {...props} />
  ),
  a: (props: React.ComponentProps<"a">) => (
    <a className="text-brand-blue font-semibold hover:text-brand-navy underline underline-offset-2" {...props} />
  ),
  ...
  blockquote: (props: React.ComponentProps<"blockquote">) => (
    <blockquote className="border-l-4 border-brand-sky bg-surface px-5 py-3 my-6 text-brand-navy italic" {...props} />
  ),
  code: (props: React.ComponentProps<"code">) => (
    <code className="rounded bg-surface px-1.5 py-0.5 text-sm font-mono text-brand-navy" {...props} />
  ),
  pre: (props: React.ComponentProps<"pre">) => (
    <pre className="rounded-xl bg-brand-navy text-white p-4 my-6 overflow-x-auto text-sm" {...props} />
  ),
    ...
    <>
      ...
      <section className="relative bg-[linear-gradient(180deg,#FFFFFF_0%,#F5FAFF_100%)]">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-brand-navy leading-[1.1]">
            {post.title}
          </h1>
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          ...
          <aside className="mt-14 rounded-2xl bg-surface border border-card-border p-6">
            ...
            <ul className="mt-3 space-y-2 text-sm">
              <li>
                <Link href="/services/vrv-vrf-maintenance" className="text-brand-blue font-semibold hover:text-brand-navy">
                  VRV/VRF Preventive Maintenance →
                </Link>
              </li>
              <li>
                <Link href="/services/emergency-vrf-repair" className="text-brand-blue font-semibold hover:text-brand-navy">
                  24/7 Emergency VRF Repair →
                </Link>
              </li>
              <li>
                <Link href="/services/daikin-vrv-service" className="text-brand-blue font-semibold hover:text-brand-navy">
                  Daikin VRV Service →
                </Link>
              </li>
            </ul>
          ...
          {post.faqs && post.faqs.length > 0 && (
            <section className="mt-14" aria-labelledby="post-faq">
              <h2 id="post-faq" className="text-2xl sm:text-3xl font-bold text-brand-navy mb-6">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          <div className="rounded-2xl bg-white border border-card-border p-8 sm:p-12 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="max-w-2xl">
              <h2 id="post-cta" className="text-3xl sm:text-4xl font-bold text-brand-navy">
